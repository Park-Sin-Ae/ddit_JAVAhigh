...
//		System.out.println("Set의 데이터들 >> " + hs1.toString());	// toString 생략 가능
		...
		// Set의 데이터를 수정하려면 수정하는 명령(메서드)가 따로 없기 때문에
...
//		System.out.println("clear 후 set 데이터 >> " + hs1);
		...
		/*
		 * Set의 데이터들은 순서(index) 가 없기 때문에 List 처럼 index로 데이터를 하나씩 꺼내올 수 없다.
		 * 그래서 데이터를 하나씩 차례로 얻기 위해서는 Iterator형 객체로 변환해야 한다.
		 * - Set형의 데이터를 Interator형 객체로 변환하는 메서드 ==> iterator()
		 */
		
		Iterator it = hs1.iterator();	// Set 데이터를 Iterator로 변환하기
		
		// Iterator의 hasNext()메서드
		// 		==> Iterator의 포인터가 가리키는 곳의 다음 번째에 데이터가 있는지 검사한다.
		//		==> 반환값 : 다음번째에 데이터가 있으면 true, 없으면 false
		
		while(it.hasNext()) {
			// Iterator의 next()메서드
			// ==> Iterator의 포인터를 다음번째 위치로 이동 한 후 그 곳의 데이터를 반환한다.
			System.out.println(it.next());
		}
		   System.out.println("향상된 for문 이용하여 Set의 데이터 출력");
		      for (Object obj : hs1) {
		...
		// 시작값 부터 종료값 사이의 정수형 난수 만들기
		// (int)(Math.randon() * (종료값 - 시작값+1) + 시작값)
